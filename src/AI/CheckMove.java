...
public class CheckMove {
    ...
    public void checkObstaclesAndBoundary(Player tank, Actor[][] map, ArrayList<LifePack> lifePacks, ArrayList<CoinPack> coinPacks) {
    ...
    public void CreateValueMap(Actor destination, Player player) {
        ...
        if (destination.getType() != "O") {
            ...
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    valueMap[i][j] = valueMap[i][j] + 30 - Math.abs(pointrxX + j) - Math.abs(pointrxY + i);
                }
            }
        } else {
